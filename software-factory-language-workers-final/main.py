...
def approval_approve(approval_id:str,response:str=typer.Option('Approved','--response')):
 ...
 if resolved.requested_action == 'flutter pub get':
  state=s.run(resolved.project_id)
  typer.echo(f'Resumed workflow: {state.current_state.value}')
...
def doctor():
 ...
 print(f'Flutter: {version("flutter")}')
 print(f'Dart: {version("dart")}')
 print(f'Git: {version("git")}')
 configured = bool(s.api_key) if s.provider != 'mock' and s.mode not in ('mock','dry-run') else True
 print(f'LLM provider: {s.provider} | model: {s.model} | configured: {"yes" if configured else "no"}')
 print(f'Factory mode: {s.mode}')
 print(f'Database: {s.db_path} | exists={s.db_path.exists()}')
 print(f'Workspace root: {s.workspaces_root} | exists={s.workspaces_root.exists()}')
 if s.provider != 'mock' and s.mode not in ('mock','dry-run') and not s.api_key:
  print('LLM configuration error: AI_API_KEY is not configured.')

@app.command('ai-smoke')
def ai_smoke():
 s=get_settings()
 if s.mode in ('mock','dry-run') or s.provider == 'mock':
  typer.echo(f'Provider: {s.provider} | model: {s.model} | configured: yes | mock: yes')
  typer.echo('Smoke test skipped network: Mock provider is active.')
  return
 if not s.api_key:
  raise typer.BadParameter('AI_API_KEY is not configured for the selected real provider')
 ...
 schema={'type':'object','properties':{'ok':{'type':'boolean'},'message':{'type':'string'}},'required':['ok','message']}
 try:
  ...
  typer.echo(f'Provider: {s.provider} | model: {s.model} | configured: yes | structured: yes')
  typer.echo('Connection: OK')
  typer.echo(f"Structured response valid: {bool(result.get('ok') is True and isinstance(result.get('message'), str))}")
 except Exception as e:
  typer.echo(f'Provider: {s.provider} | model: {s.model} | configured: yes')
  typer.echo(f'Connection: FAILED | {type(e).__name__}: {str(e).replace(s.api_key or "", "[REDACTED]")}')
...
def health():
 ...
 def alive(name, max_age=30):
  ...
  try: return (datetime.now(timezone.utc)-datetime.fromisoformat(hb[0])).total_seconds() <= max_age and json.loads(hb[1]).get('status') == 'running'
 ...
 for wt in ('python','node','java','flutter'):
  typer.echo(f'{wt.title()} Environment: {"OK" if alive(f"worker:{wt}") else "NOT REQUIRED/NOT REPORTED"}')
 typer.echo(f'Database: OK')
 typer.echo(f'Queue: OK | pending={db.queue_count()}')
 typer.echo(f'AI Provider: {"OK" if ai["configured"] else "NOT CONFIGURED"} | {ai["provider"]}/{ai["model"]}')
