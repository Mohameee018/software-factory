...
TRANSITIONS={
 ...
 # A blocked project may need to return to intake so missing/unknown metadata can be
 # re-discovered on retry. This is intentional recovery, not a normal forward transition.
 WorkflowState.BLOCKED:{WorkflowState.IDEA,WorkflowState.PLANNING,WorkflowState.TASK_CREATION,WorkflowState.IMPLEMENTATION,WorkflowState.FIXING,WorkflowState.CANCELLED,WorkflowState.PAUSED},
