...
class TelegramBot:
    def __init__(self, service, settings):
        ...
        self.application.add_handler(CommandHandler('start',self._guard(h.start)))
        self.application.add_handler(CommandHandler('help',self._guard(h.help)))
        self.application.add_handler(CommandHandler('new',self._guard(h.new)))
        for cmd,fn in [('projects',h.projects),('project',h.project),('status',h.status),('manager',h.manager),('requirements',h.requirements),('tasks',h.tasks),('run',h.run),('pause',h.pause),('resume',h.resume),('cancel',h.cancel),('retry',h.retry),('logs',h.logs),('approve',h.approve),('reject',h.reject),('review',h.review),('feedback',h.feedback),('github_public',h.github_public),('ai_smoke',h.ai_smoke),('audit',h.audit)]:
            self.application.add_handler(CommandHandler(cmd,self._guard(fn)))
    ...
    def _guard(self, fn):
        async def wrapped(update, context):
            if update.effective_chat:
                try: self.service.db.register_telegram_chat(update.effective_chat.id)
