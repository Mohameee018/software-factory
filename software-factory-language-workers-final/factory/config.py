...
class Settings:
    ...
    mode: str = 'auto'; ai_timeout: int = 120; database_url: str|None = None; worker_poll_interval: float = 2.0; job_max_retries: int = 5; worker_type: str = 'generic'; max_agent_runs: int = 250; max_project_seconds: int = 0; dashboard_port: int = 8080; dashboard_token: str|None = None
...
def load_settings():
    ...
    return Settings(
      ...
      os.getenv('DATABASE_URL'), float(os.getenv('WORKER_POLL_INTERVAL','2')), int(os.getenv('JOB_MAX_RETRIES','5')), os.getenv('WORKER_TYPE','generic').lower(), int(os.getenv('MAX_AGENT_RUNS','250')), int(os.getenv('MAX_PROJECT_SECONDS','0')), int(os.getenv('DASHBOARD_PORT','8080')), os.getenv('DASHBOARD_TOKEN'))
_settings=None
def get_settings():
    global _settings
