...
class Task(BaseModel):
    id:str=Field(default_factory=lambda:new_id('task')); project_id:str; title:str; description:str; status:TaskStatus=TaskStatus.PENDING; priority:Priority=Priority.MEDIUM; dependencies:list[str]=Field(default_factory=list); assigned_agent:str|None=None; acceptance_criteria:list[str]=Field(default_factory=list); files_expected:list[str]=Field(default_factory=list); tests_required:list[str]=Field(default_factory=list); skills:list[str]=Field(default_factory=list); retry_count:int=0; created_at:datetime=Field(default_factory=now); updated_at:datetime=Field(default_factory=now); completed_at:datetime|None=None; failure_reason:str|None=None
